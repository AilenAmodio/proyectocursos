...
public class AlumnoController {
	...
	/*@RequestMapping(value = "/editar/{dni}", method = { RequestMethod.POST, RequestMethod.PUT })
	public String edit( @RequestParam(value = "nombre") String nombre,
			@RequestParam(value = "apellido") String apellido, @RequestParam(value = "dni") Long dni,
			@RequestParam(value = "vacante", required = false) Boolean activo, Model model) throws ParseException {
...
//		return "redirect:/listado";
	...
	public String borrar(@RequestParam (value= "dni") Long dni, Model model) {
		Alumno alumno = alumnoRepo.findByDni(dni).get();
		model.addAttribute("alumno", alumno);
	...
	@PostMapping(value="borrar/{dni}")
	public String borrarAlumno(@PathVariable(value="dni") Long dni, Model model) {
//
		try {
